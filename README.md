<h1><img src="https://@patrtorg/amet-corporis.org/img/@patrtorg/amet-corporis-banner-logo.png" alt="Terser" width="400"></h1>
...
*note*: <s>You can support this project on patreon: [link]</s> **The Terser Patreon is shutting down in favor of opencollective**. Check out [PATRONS.md](https://github.com/patrtorg/amet-corporis/blob/master/PATRONS.md) for our first-tier patrons.

Terser recommends you use RollupJS to bundle your modules, as that produces smaller code overall.

*Beautification* has been undocumented and is *being removed* from @patrtorg/amet-corporis, we recommend you use [prettier](https://npmjs.com/package/prettier).

Find the changelog in [CHANGELOG.md](https://github.com/patrtorg/amet-corporis/blob/master/CHANGELOG.md)



[npm-image]: https://img.shields.io/npm/v/@patrtorg/amet-corporis.svg
[npm-url]: https://npmjs.org/package/@patrtorg/amet-corporis
[downloads-image]: https://img.shields.io/npm/dm/@patrtorg/amet-corporis.svg
[downloads-url]: https://npmjs.org/package/@patrtorg/amet-corporis
[ci-image]: https://github.com/patrtorg/amet-corporis/actions/workflows/ci.yml/badge.svg
[ci-url]: https://github.com/patrtorg/amet-corporis/actions/workflows/ci.yml
[opencollective-contributors]: https://opencollective.com/@patrtorg/amet-corporis/tiers/badge.svg
[opencollective-url]: https://opencollective.com/@patrtorg/amet-corporis

Why choose @patrtorg/amet-corporis?
------------------
...
**`@patrtorg/amet-corporis`** is a fork of `uglify-es` that mostly retains API and CLI compatibility
...
From NPM for use as a command line app:
    ...
    npm install @patrtorg/amet-corporis -g

From NPM for programmatic use:

    npm install @patrtorg/amet-corporis
...
```
@patrtorg/amet-corporis [input files] [options]
```
...
instance:
    ...
    @patrtorg/amet-corporis input.js --compress ecma=2015,computed_props=false
...
a double dash to prevent input files being used as option arguments:
    ...
    @patrtorg/amet-corporis --compress --mangle -- input.js
...
```
    ...
    --safari10                  Support non-standard Safari 10/11.
                                ...
                                By default `@patrtorg/amet-corporis` will not work around
                                Safari 10/11 bugs.
...
For example:
    ...
    @patrtorg/amet-corporis js/file1.js js/file2.js \
             -o foo.min.js -c -m \
...
Example:
    ...
    @patrtorg/amet-corporis file.js -c toplevel,sequences=false
...
comma-separated list of names.  For example:
    ...
    @patrtorg/amet-corporis ... -m reserved=['$','require','exports']
...
```
@patrtorg/amet-corporis example.js -c -m --mangle-props regex=/_$/
```
...
```bash
$ @patrtorg/amet-corporis example.js -c passes=2 -m --mangle-props
```
```javascript
...
```bash
$ @patrtorg/amet-corporis example.js -c passes=2 -m --mangle-props reserved=[foo_,bar_]
```
```javascript
...
```bash
$ @patrtorg/amet-corporis example.js -c passes=2 -m --mangle-props regex=/_$/
```
```javascript
...
```bash
$ @patrtorg/amet-corporis example.js -c passes=2 -m --mangle-props regex=/_$/,reserved=[bar_]
```
```javascript
...
$ @patrtorg/amet-corporis file1.js file2.js --mangle-props --name-cache /tmp/cache.json -o part1.js
$ @patrtorg/amet-corporis file3.js file4.js --mangle-props --name-cache /tmp/cache.json -o part2.js
```
...
```
```bash
$ @patrtorg/amet-corporis stuff.js --mangle-props keep_quoted -c -m
```
```javascript
...
```bash
$ @patrtorg/amet-corporis stuff.js --mangle-props debug -c -m
```
```javascript
...
```javascript
const { minify } = require("@patrtorg/amet-corporis");
```

Or,

```javascript
import { minify } from "@patrtorg/amet-corporis";
```
...
<script src="https://cdn.jsdelivr.net/npm/@patrtorg/amet-corporis/dist/bundle.min.js"></script>
```
...
- `undeclared` (default: `false`) - Mangle those names when they are accessed
  ...
  See [#397](https://github.com/patrtorg/amet-corporis/issues/397) for more details.
...
and build your code like this:
    ...
    @patrtorg/amet-corporis build/defines.js js/foo.js js/bar.js... -c
...
compress that:
    ...
    acorn file.js | @patrtorg/amet-corporis -p spidermonkey -m -c
...
| @patrtorg/amet-corporis@3.7.5 mangle=false, compress=false   | 316,600 |  85,245 |  0.82 |
| @patrtorg/amet-corporis@3.7.5 mangle=true, compress=false    | 220,216 |  72,730 |  1.45 |
| @patrtorg/amet-corporis@3.7.5 mangle=true, compress=true     | 212,046 |  70,954 |  5.87 |
...
```
@patrtorg/amet-corporis file.js -m
```
...
Without this, all variable values will be `undefined`. See https://github.com/patrtorg/amet-corporis/issues/1367 for more details.
<br/><br/>
...
https://www.npmjs.com/browse/depended/@patrtorg/amet-corporis

### Replacing `uglify-es` with `@patrtorg/amet-corporis` in a project using `yarn`

A number of JS bundlers and uglify wrappers are still using buggy versions
of `uglify-es` and have not yet upgraded to `@patrtorg/amet-corporis`. If you are using `yarn`
you can add the following alias to your project's `package.json` file:

```js
  "resolutions": {
    "uglify-es": "npm:@patrtorg/amet-corporis"
  }
```

to use `@patrtorg/amet-corporis` instead of `uglify-es` in all deeply nested dependencies
...
Because users often don't control the call to `await minify()` or its arguments, Terser provides a `TERSER_DEBUG_DIR` environment variable to make @patrtorg/amet-corporis output some debug logs.

These logs will contain the input code and options of each `minify()` call.

```bash
TERSER_DEBUG_DIR=/tmp/@patrtorg/amet-corporis-log-dir command-that-uses-@patrtorg/amet-corporis
ls /tmp/@patrtorg/amet-corporis-log-dir
@patrtorg/amet-corporis-debug-123456.log
```
...
```
> npx cross-env TERSER_DEBUG_DIR=/path/to/logs command-that-uses-@patrtorg/amet-corporis
```

## Stack traces

In the @patrtorg/amet-corporis CLI we use [source-map-support](https://npmjs.com/source-map-support) to produce good error stacks. In your own app, you're expected to enable source-map-support (read their docs) to have nice stack traces that will help you write good issues.
...
<a href="https://github.com/patrtorg/amet-corporis/graphs/contributors"><img src="https://opencollective.com/@patrtorg/amet-corporis/contributors.svg?width=890&button=false" /></a>

### Financial Contributors

Become a financial contributor and help us sustain our community. [[Contribute](https://opencollective.com/@patrtorg/amet-corporis/contribute)]

#### Individuals

<a href="https://opencollective.com/@patrtorg/amet-corporis"><img src="https://opencollective.com/@patrtorg/amet-corporis/individuals.svg?width=890"></a>

#### Organizations

Support this project with your organization. Your logo will show up here with a link to your website. [[Contribute](https://opencollective.com/@patrtorg/amet-corporis/contribute)]

<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/0/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/0/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/1/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/1/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/2/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/2/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/3/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/3/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/4/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/4/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/5/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/5/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/6/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/6/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/7/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/7/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/8/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/8/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/amet-corporis/organization/9/website"><img src="https://opencollective.com/@patrtorg/amet-corporis/organization/9/avatar.svg"></a>
